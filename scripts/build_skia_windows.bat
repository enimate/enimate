@echo off
REM Build Skia for Windows (CPU-only backend)
REM Creates a static library for Windows native compilation
...
echo === Skia Build Script for Windows (CPU Backend) ===
...
set "BUILD_DIR=out\enimate_windows_release"
...
echo win_vc = "/c/Program Files/Microsoft Visual Studio/2022/Community/VC/Tools/MSVC"
echo.
echo # Optimization
echo symbol_level = 1
) > "%BUILD_DIR%\args.gn"
...
copy "%BUILD_DIR%\skia.lib" "%PROJECT_ROOT%\native\lib\skia_windows.lib"
if errorlevel 1 (
    echo Error: Failed to copy library
    exit /b 1
)
echo skia_windows.lib copied to native\lib\
